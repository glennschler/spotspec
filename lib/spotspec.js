...
* @arg {string[]} [options.InstanceTypes] - Array of instance types to be priced
* @arg {string} [options.product=Linux/UNIX] - e.g. 'Windows'
* @arg {string[]} [options.ProductDescriptions] - Array of ProductDescriptions
...
SpotSpec.prototype.prices = function (options) {
  ...
  let products = options.product ? [options.product] : undefined
  let params = {
  ...
  if (!params.ProductDescriptions) {
    params.ProductDescriptions = ['Linux/UNIX'] // set a default
  }

  if (!params.InstanceTypes) {
    delete params.InstanceTypes   // ec2 will give all types
  }
...
SpotSpec.prototype.terminateInstances = function terminateInstances (options) {
  ...
  req.on('error', function onError (err) {
    internals.logger.warn('Instance Terminate Error:\n', err)
    Intern.emitNextTick.call(self, Const.EVENT_TERMINATED, err)
  })
  ...
  req.on('success', function onSuccess (resp) {
    ...
    */
    Intern.emitNextTick.call(self, Const.EVENT_TERMINATED,
                              null, termInstances)
  })
...
SpotSpec.prototype.cancelSpotRequest = function cancelSpotRequest (options) {
  ...
  req.on('error', function onError (err) {
    internals.logger.warn('Instance Terminate Error:\n', err)
    Intern.emitNextTick.call(self, Const.EVENT_CANCELED, err)
  })
  ...
  req.on('success', function onSuccess (data) {
    ...
    */
    Intern.emitNextTick.call(self, Const.EVENT_CANCELED, null, canceledArr)
  })
