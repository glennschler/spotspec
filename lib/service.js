...
* @private
*/
internals.Credentials.prototype.update = function (constructOps) {
  var self = this
  ...
  req.on('success', function (response) {
    ...
    if (!data.hasOwnProperty('Credentials')) {
      internals.logger.info('headers, but no data streamed: ' +
        JSON.stringify(response.httpResponse.headers))

      /**
      * @todo Issue using node-inspector with node v 0.12.x and iojs 3.x
      */
